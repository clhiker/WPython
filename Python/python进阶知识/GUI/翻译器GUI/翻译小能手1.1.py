...
class FanYiQi():
    ...
    def interpreter(self):
        ...
        try:
            ...
            if len(dir) == 68:
                self.output_text.delete('0.0', END)
                self.output_text.insert('insert', '要输入东西啊<_>')
            # from = to
            elif self.from_language == self.to_language:
                self.output_text.delete('0.0', END)
                self.output_text.insert('insert', '翻来翻去都一样-_-')
            ...
            self.output_text.delete('0.0', END)
            self.output_text.insert('insert', '404^_^')
